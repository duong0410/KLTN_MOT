#!/usr/bin/env python3
"""
ByteTrack using BoxMOT library (official implementation)
This script uses the official BoxMOT library to run ByteTrack on MOT17 dataset
...
# Import ByteTrack from BoxMOT (correct for version 16.0.7)
try:
    from boxmot import ByteTrack
    print(" BoxMOT ByteTrack imported successfully!")
except ImportError as e:
    print(f" Cannot import ByteTrack from boxmot: {e}")
...
def track_mot17_with_boxmot(dataset, output_dir, args):
    """
    Track MOT17 using BoxMOT's ByteTrack
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print("\n" + "="*80)
    print("Running ByteTrack (BoxMOT Library v16.0.7)")
    print("="*80)
    ...
    for seq_name in dataset.sequences:
        ...
        # BoxMOT 16.0.7 ByteTrack parameters
        tracker = ByteTrack(
            track_thresh=args.track_thresh,      # High confidence threshold
        ...
        for frame_id in range(1, num_frames + 1):
            ...
            # Update tracker
            # BoxMOT ByteTrack.update() requires: update(dets, img)
            # Create dummy image (not used in ByteTrack but required by API)
            ...
            if tracks is not None and len(tracks) > 0:
                for track in tracks:
                    # ByteTrack output format: [x1, y1, x2, y2, track_id, conf, cls, det_ind]
                    if len(track) >= 5:
...
def compute_mot_metrics(data_root, pred_dir):
    ...
    for detector in ['DPM', 'FRCNN', 'SDP']:
        ...
        print(f"\n{detector} summary index: {list(summary.index)}")
...
def save_detailed_summary(pred_dir, detector_metrics, summary_results, args):
    ...
    with open(summary_file, 'w') as f:
        f.write("="*100 + "\n")
        f.write(f"{'MOT17 EVALUATION RESULTS - ByteTrack (BoxMOT Library)':^100}\n")
        f.write("="*100 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='ByteTrack using BoxMOT library on MOT17')
    ...
    parser.add_argument('--output_dir', type=str, 
                       default='results_boxmot_bytetrack',
                       help='Directory to save tracking results')
    
    # BoxMOT ByteTrack parameters
    ...
    print("="*80)
    print("ByteTrack using BoxMOT Library - MOT17 Benchmark")
    print("="*80)
    ...
    # Run tracking
    track_mot17_with_boxmot(dataset, args.output_dir, args)
